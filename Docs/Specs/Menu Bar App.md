...
- **`minCycles` raises that floor for one clip at a time.** A looping clip may declare a minimum number of *full cycles* that must play before any swap lands (`nil` means 1 — today's behaviour). `happy` sets 8, because at 1.6s a single cycle is over before the eye has read it. The cost is real and deliberate: a genuine state change is deferred by up to `minCycles × duration` — 12.8s for `happy` — which is the same order as the 10.16s a single cycle of `done-flag` already costs. Power transitions still bypass this, so wake and power-off stay immediate.
