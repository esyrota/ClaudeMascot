...
- **The departure is bounded** by `PanelTimings.leaveBy` (20s), and abandoned outright if no route off the panel exists — `sitting` is still an island, so a session ending mid-tool goes dark where it sits. A mascot that cannot finish leaving must not hold the panel lit forever.
